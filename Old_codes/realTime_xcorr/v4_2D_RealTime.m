...
% solucoes sao muito lentas!   gera grid para ter o x0
% 
%
...
function solutions = solve_hyperbola_pairs(t12, t13, t14, mic_pos, c)
    ...
    % Usa a versão rápida com fsolve
    sol12_13 = solve_pair_fsolve(mic_pos(1,:), mic_pos(2,:), d12, ...
                                 mic_pos(1,:), mic_pos(3,:), d13);
    
    sol12_14 = solve_pair_fsolve(mic_pos(1,:), mic_pos(2,:), d12, ...
                                 mic_pos(1,:), mic_pos(4,:), d14);
    
    sol13_14 = solve_pair_fsolve(mic_pos(1,:), mic_pos(3,:), d13, ...
                                 mic_pos(1,:), mic_pos(4,:), d14);

    solutions = [sol12_13; sol12_14; sol13_14];
end

function points = solve_pair_fsolve(m1, m2, d1, m3, m4, d2)
    points = [];

    % Define função de sistema com dois hiperbolas
    f = @(p) [
        norm(p - m1) - norm(p - m2) - d1;
        norm(p - m3) - norm(p - m4) - d2
    ];

    % Gerar vários palpites iniciais (grid perto dos microfones)
    guesses = generate_guesses([m1; m2; m3; m4]);

    for i = 1:size(guesses, 1)
        try
            opts = optimoptions('fsolve', 'Display', 'off', 'FunctionTolerance', 1e-5);
            sol = fsolve(f, guesses(i,:), opts);
            if all(isreal(sol)) && ~any(isnan(sol))
                % Verifica se solução é nova (evita duplicadas)
                if isempty(points) || min(vecnorm(points - sol, 2, 2)) > 1e-3
                    points(end+1,:) = sol;
                end
            end
        catch
            % Ignora falhas de convergência
        end
    end
end

function guesses = generate_guesses(mics)
    % Gerar alguns palpites iniciais perto do centro dos microfones
    center = mean(mics, 1);
    span = max(pdist(mics));  % extensão máxima entre microfones

    % Grid de palpites em cruz
    offsets = [0, 0;
               0.05, 0;
              -0.05, 0;
               0, 0.05;
               0, -0.05];
    guesses = center + offsets;
end
