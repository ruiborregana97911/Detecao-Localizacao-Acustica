...
tmp_ball_sound= 30;     %tempo do som da bola em ms
...
init_ref=600;   %necessario visto que o inicio nao e logo o som da bola!!!
ref_audio=ref_audio(init_ref:sample_range+init_ref);    %USAR ISTO PARA COMP O SINAL COM A REF
                ...
                if(eng_prev > threshold && eng_prev > eng && eng_prev > eng_prev2)
                    ...
                    if(count >= sample_range)
                        ...
                        if(channel == 1)
                            ...
                            [pos_s, c2, lags2]=calculate_pos_1D_v3(data1, data2, Fs, v);   
                        else
                            [c,lags] = xcorr(data_norm(2,:), ref_audio_norm(1:sample_range));
                            c=c/sample_range;
                            [~,max_c]=max(abs(c));
                            t_lag=lags(max_c);
...
if enable_debub
    ...
    %windowed sound
    %t_ws=linspace(0,tmp_ball_sound,sample_range);
    j=1;
    t_a = ((0:length(array_sound)-1)/Fs)*10^3;
    for i=1:(debug_aux-1)/2
        figure;
        plot(t_a,array_sound(j,:));
        j=j+1;
        hold on;
        plot(t_a,array_sound(j,:));
        j=j+1;
