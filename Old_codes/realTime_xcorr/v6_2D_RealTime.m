...
% debuging problema de uma das hisperboles nao dar !
% 
% colocque a janela de hamming
%
...
init_ref=24200;   %necessario visto que o inicio nao e logo o som da bola!!!(ex:600)
...
subplot(4,1,1); plot(ss1(1:500)); title('Canal 1'); xlabel('Tempo (s)');
subplot(4,1,2); plot(ss2(1:500)); title('Canal 2'); xlabel('Tempo (s)');
subplot(4,1,3); plot(ss3(1:500)); title('Canal 3'); xlabel('Tempo (s)');
subplot(4,1,4); plot(ss4(1:500)); title('Canal 4'); xlabel('Tempo (s)');

figure;
plot(ss1,'DisplayName', 'channel 1');
hold on;
plot(ss2,'DisplayName', 'channel 2');
hold on;
plot(ss3,'DisplayName', 'channel 3');
hold on;
plot(ss4,'DisplayName', 'channel 4');
hold off;
legend show;

figure;
plot(lags_plot, abs(c_plot));
...
function [x, resnorm, f] = locate_2D_NLS2(t12, t13, t14, mic_pos, c, mic)
   ...
   % Começa na posição do microfone que detectou primeiro
    ...
    opts = optimoptions('lsqnonlin', 'Display','iter','MaxIterations', 20); 
    lower_bounds = mic_pos(1,:);
    upper_bounds = mic_pos(4,:);
    %[x, resnorm] = lsqnonlin(f, x0, [], [], opts);
    [x, resnorm] = lsqnonlin(f, x0, lower_bounds, upper_bounds, opts);
