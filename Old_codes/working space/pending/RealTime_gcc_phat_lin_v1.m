...
% prog de tentativa de localizacao com linearizacao do problema
% 
%
...
num_channels = 4;
threshold = 0.5e-4;           % limite para deteccao de impacto
            ...
            for channel= 1:4
                ...
                if(eng_prev > threshold && eng_prev > eng && eng_prev > eng_prev2)
                    ...
                    if(count >= sample_range)
                        ...
                        t12 = gccphat(data1_norm.', data2_norm.', Fs);
                        t13 = gccphat(data1_norm.', data3_norm.', Fs);
                        t14 = gccphat(data1_norm.', data4_norm.', Fs);
                        ...
                        tdoa = [t12;t13;t14];
                        pos_est = locate_tdoa_rt(tdoa, c, Cinv, n, ref_mic);
...
%%
function pos = locate_tdoa_rt(tdoa, c, Cinv, n, ref)
    Delta = c * tdoa(:);             % (3x1)
    r = 0.5 * (n - Delta.^2);        % (3x1)
    pos_rel = Cinv * r;              % (2x1)
    pos = pos_rel' + ref;            % (1x2)
