...
op_time = 25;              % tempo de operacao (em segundos)
...
margem_mesa = 0.05;     %margem para alem dos limites da mesa
...
% --- Estrutura para guardar localizações estimadas ---
resultados = struct('tempo_s', {}, 'x_est', {}, 'y_est', {});
                ...
                if(eng_prev > threshold && eng_prev > eng && eng_prev > eng_prev2)
                    ...
                    if(count >= sample_range)
                        ...
                        if (pos_est(1) <= d_x+margem_mesa && pos_est(2) <= d_y+margem_mesa && ...
                            ...
                            % Guardar dados do impacto
                            novo_resultado.tempo_s = peak_position / Fs;
                            novo_resultado.x_est = pos_est(1);
                            novo_resultado.y_est = pos_est(2);
                            resultados(end+1) = novo_resultado;
...
%%
% --- Guardar resultados ---
% Define pasta onde guardar os resultados
pasta_resultados = fullfile(pwd, 'resultados_localizacao');  
if ~exist(pasta_resultados, 'dir')
    mkdir(pasta_resultados);
end

% Informações da medição
% x_real = 0;   % <-- substitui pelo valor real da posição X (em cm ou m)
% y_real = 5;   % <-- substitui pelo valor real da posição Y
% descricao = sprintf('x%.0f_y%.0f', x_real, y_real);  % nome limpo, em cm

% Nome do ficheiro
timestamp = datestr(now, 'yyyy-mm-dd_HH-MM-SS');
nome_ficheiro = sprintf('result_30_%s.mat', timestamp);

% Caminho completo
caminho_completo = fullfile(pasta_resultados, nome_ficheiro);

% Guardar
save(caminho_completo, 'resultados');
