function features = extract_features_audio_v2(audio, Fs)
    
    %Feature 1: zero crossing rate
    signs = sign(audio);
    signs(signs == 0) = 1;  %evita "falsos positivos" se o sinal cruzar exatamente por zero e ficar um tempo parado
    sign_changes = diff(signs);
    crossings = sum(sign_changes ~= 0);
    zcr = Fs * crossings / length(audio);
    ...
    %Feature 3: Clearence factor
    peak= max(abs(audio));
    CF = peak / (mean(sqrt(abs(audio)))^2);
    ...
    %Feature 5: Impulse factor
    ifac = peak / mean(abs(audio));
    ...
    %Feature 7: razao pico max / pico sec    
        ...
        if length(sorted_peaks) >= 2 && sorted_peaks(2) > 0
            ...
            ratio_peaks = NaN;  % ou 1, dependendo do que fizer mais sentido para ti
        end
    ...
    %Feature 10: Shape Factor
    sf = sqrt(mean(audio.^2)) / mean(abs(audio));
    ...
    magnitude_dB = 20 * log10(magnitude_norm + eps);  % em dBs  (eps evita log(0))
    ...
    % Ordena por frequência crescente (opcional, mas bom para consistência)
    freqs_top_sorted = sort(freqs_top);
    
    
    %Feature 12: Spectral Entropy       
    ps = magnitude.^2;   %power spectral density        % 1/N .* 
    ps_sum = sum(ps);
    ps_norm = ps / ps_sum;  % normalizar
    spec_entropy = -sum(ps_norm .* log2(ps_norm + eps)) / log2(length(ps)); %escala de [0,1]
    ...
    %Feature 16: Spectral skewness
    f_diff= f_half - spec_centroid;
    ps_sum_weighted2 = sum(ps .* f_diff.^2);
    ps_sum_weighted3 = sum(ps .* f_diff.^3);
    
    spec_skewness= (ps_sum_weighted3 / ps_sum) / (sqrt(ps_sum_weighted2 / ps_sum))^3;
    ...
    %Feature 18: Spectral variance
    spec_variance= ps_sum_weighted2 / ps_sum;
    ...
    %Feature 22: Energy Entropy(shannon entropy aplied!)
    
    n_blocks = 10;
    block_len = floor(length(audio) / n_blocks);
    energy_blocks = zeros(1, n_blocks);
    
    for i = 1:n_blocks
        idx = (i-1)*block_len+1 : i*block_len;
        energy_blocks(i) = sum(audio(idx).^2);
    end
    
    % Normaliza as energias
    energy_blocks = energy_blocks / sum(energy_blocks + eps);
    
    % Entropia
    entropy_energy = -sum(energy_blocks .* log2(energy_blocks + eps));
    
    
    features = [zcr, rms_val, CF, cf, ifac, waveform_length, ratio_peaks, ...
        kurtosis, skewness, sf, spec_entropy, rolloff_freq, spec_flatness, ...
        spec_centroid, spec_skewness, spec_kurtosis, spec_variance, ...
        spec_bandwidth, tempo_ataque_ms, tempo_decay_ms, entropy_energy, ...
        freqs_top_sorted];
