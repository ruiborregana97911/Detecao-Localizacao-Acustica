...
% prog de debug para o correlacao cruzada dos sinais, guardar os sinais
% quando estao mal 
% 
%
...
op_time = 60;              % tempo de operacao (em segundos)
d_y = 0.5;                 % distancia maxima em Y (m)
d_x = 0.8;                 % distancia maxima em X (m)
...
num_channels = 4;
threshold = 0.2e-3;           % limite para deteccao de impacto
...
debug_count = 1;    %contador para nomear variaveis guardadas
%end of debug
            ...
            for channel= 1:4
                ...
                if(eng_prev > threshold && eng_prev > eng && eng_prev > eng_prev2)
                    ...
                    if(count >= sample_range)
                        ...
                        % [t12, cc12, lags12] = estimate_tdoa(data1_norm, data2_norm, Fs);
                        % [t13, cc13, lags13] = estimate_tdoa(data1_norm, data3_norm, Fs);
                        % [t14, cc14, lags14] = estimate_tdoa(data1_norm, data4_norm, Fs);

                        [t12, cc12, lags12] = gcc_phat(data1_norm, data2_norm, Fs);
                        [t13, cc13, lags13] = gcc_phat(data1_norm, data3_norm, Fs);
                        [t14, cc14, lags14] = gcc_phat(data1_norm, data4_norm, Fs);
                        ...
                        if(abs(t12) > max_deviation || abs(t13) > max_deviation || abs(t14) > max_deviation)
                            filename="debub_vars" + debug_count;
                            save(filename, "data1_norm", "data2_norm", "data3_norm", "data4_norm", ...
                                "cc12", "cc13", "cc14", "lags12", "lags13", "lags14", "t12", "t13", "t14", ...
                                "pos_est");
                            disp('Vars saved!!');
