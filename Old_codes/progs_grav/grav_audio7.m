...
wind_size=Fs/20;
wind_num=length(audio_data1)/wind_size;

Eng_med1=zeros(wind_num,1);
...
tmp_analise=0.083;  %segundos
range= ceil(tmp_analise*Fs); %tamanho do frame de analise, com arrendondamento para +inf
...
for i=1:length(delay_time)
    ...
    if((x(i)/2 <= 25) && (x(i)/2 >= -25))
        pos_sound(i)= x(i)/2; %posicao em relacao ao ponto medio
        fprintf('Posição do som nº %d: %.3f cm\n', i, pos_sound(i));
    end
end

%% fazer plot dos sianis de audio
t_audio=linspace(0,rec_time,length(audio_data1));
figure;
subplot(2,1,1);
plot(t_audio,audio_data1);
xlabel('Tempo [s]');
ylabel('Amplitude');
title('Audio 1');
grid on;
subplot(2,1,2);
plot(t_audio,audio_data2);
xlabel('Tempo [s]');
ylabel('Amplitude');
title('Audio 2');
grid on;
...
% %% filtro 
% 
% fc = 15000; %frequência de corte em Hz
% order = 4;  %ordem do filtro
% 
% %design do filtro passa-alto Butterworth
% [b, a] = butter(order, fc/(Fs/2), 'high');
% 
% figure;
% %visualizar a resposta em frequência com freqz
% freqz(b, a, 1024, Fs);
% title('Resposta em Frequência do Filtro Passa-Alto Butterworth');
% xlabel('Frequência (Hz)');
% ylabel('Magnitude (dB)');
% 
% %aplicar o filtro aos dados de áudio
% filteredAudio = filter(b, a, audio_data1);
% 
% figure;
% plot(t_audio,filteredAudio);
% xlabel('Tempo [s]');
% ylabel('Amplitude');
