...
% este codigo ja nao esta deoendente do buffer_size!!
%
%
...
range_init= buffer_size/2 - range/2;    %index do inicio da zona de analise
range_end= buffer_size/2 + range/2; %index do fim da zona de analise
...
while p_time < op_time
    ...
    if(count < (range_end/frame_size))    %garantir que comeco a analise com buffer com dados na zona de analise
        continue
    end
    ...
    mean_Eng=[mean_Eng Eng_med1];   %debbuging
        ...
        if (Eng_med1 >= threshold)
            ...
            for i=1:8
                ...
                indx_eng_init=(i-1)*(range/8)+range_init;
                indx_eng_end=i*(range/8)+range_init;   
