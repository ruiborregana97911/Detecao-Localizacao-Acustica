...
range = ceil(tmp_analise * Fs/1000); % tamanho da janela de analise
...
%tornar a analise no buffer mais dinamico
wind1_init=buffer_size/2 - range/2; %janela de analise em funcionamento normal
wind1_end= buffer_size/2 + range/2;
wind2_init= wind1_init - eng_r; %janela de analise em funcionamento especial(caso k=1)
wind2_end= wind1_end - eng_r;
...
p_time=0;
my_beep(1); %na primeira vez ele nao funciona bem
...
while p_time < op_time
    ...
    if(skip_analysis==0 && cooldown_active==0)
        ...
        if (Eng_med1 >= threshold)
            ...
            if(k>=4)
                continue
            elseif(k==1)    %fiz isto para garantir que tenho a parte importante do sinal a ser analisada
                range_init= wind2_init;
                range_end=wind2_end;
            else
                range_init= wind1_init;
                range_end=wind1_end;
            end    
...
%             %comparacao com som de referencia (retirei pra ja pois nao quero juntar pattern recognition)
