...
tmp= -5;    %tempo de deslocacao (ms)
tmp= ceil((tmp*1e-3)*Fs);
audio2 = circshift(audio1,tmp);

d = 500;                     % distancia entre microfones (cm)
...
smp_dev= ceil(max_deviation*(Fs/1000));
...
for i=1:length(audio1) 
    ...
    indx=length(mean_eng(1,:)); %index para energia
        ...
        %funciona para o audio1 como o primeiro com pico de energia     
        if(mean_eng(1,indx) > threshold && mean_eng(1,indx) < mean_eng(1,indx-1))
            ...
            if(i < sample_range)
                ...
                pos_s=calculate_pos_1D_v2(audio1(init+t_lag:init+t_lag+sample_range), ...
                    audio2(init+t_lag:init+t_lag+sample_range+smp_dev),Fs,v);
                ...
                array_sound(debug_aux,:)=audio1(init+t_lag:init+t_lag+sample_range+smp_dev);
                debug_aux=debug_aux+1;
                array_sound(debug_aux,:)=audio2(init+t_lag:init+t_lag+sample_range+smp_dev);
                debug_aux=debug_aux+1;
        ...
        % audio2 com pico de energia
        if(mean_eng(2,indx) > threshold && mean_eng(2,indx) < mean_eng(2,indx-1))
            
            if(mean_eng(2,indx-2) > mean_eng(2,indx-1))
                %disp("ignorado por pico de evento recente.");
                continue
            end
            ...
            %encontrar o sinal aqui(talvez a correlacao com o sinal de ref???)
            if(i < sample_range)
                [c,lags] = xcorr(audio2(1:i),ref_audio(1:sample_range));
                %neste caso na realidade nao faço nada!
            else
                init=i-sample_range;
                fin=i+sample_range;
                [c,lags] = xcorr(audio2(init:fin),ref_audio(1:sample_range));
                [~,max_c]=max(abs(c));
                t_lag=lags(max_c);
                

                %calculo da distancia e plot    
                pos_s=calculate_pos_1D_v2(audio1(init+t_lag:init+t_lag+sample_range+smp_dev), ...
                    audio2(init+t_lag:init+t_lag+sample_range),Fs,v);
